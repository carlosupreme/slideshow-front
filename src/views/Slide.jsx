  ...
  useEffect(() => {
    const getSlide = async () => {
      try {
        ...
        console.log(json.files[0]?.type.contains("video"));
        setSlide(json);
      } catch (e) {
        console.log(e);
      }
  ...
  return (
    <>
      <div className="h-screen w-full bg-red-500">
        {hasFiles && slide.files[0].type.contains("video") ? (
          <video className="object-cover h-full w-full" controls autoPlay>
            <source
              type="video/mp4"
              src={`${STORAGE_URL}${
          ...
          <img
            src={`${STORAGE_URL}${hasFiles ? slide.files[0].path : ""}`}
            alt=""
            className="object-cover h-full w-full" // Ensure image covers the full viewport
          />
        )}
      </div>
      ...
      <div className="fixed bottom-0 left-0 z-50 grid w-full h-24 grid-cols-1 px-8 bg-white border-t border-gray-200 md:grid-cols-3 dark:bg-gray-700 dark:border-gray-600">
        <div className="items-center justify-center hidden me-auto md:flex">
          <img
            ...
            src="https://cdn.devdojo.com/images/june2021/flowbite-logo.png"
            alt="Video preview"
          />
